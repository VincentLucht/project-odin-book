...
export default async function createOdinProjectCommunity(prisma: PrismaClient) {
  const odinPosts = [
    {
  ...
  // Helper function to create nested comments with exact count control
  const createNestedComments = async (
    postId: string,
    parentId: string | null,
    depth: number,
    maxDepth: number,
    remainingComments: { count: number },
  ) => {
    if (depth >= maxDepth || remainingComments.count <= 0) {
      return;
    }

    const numberOfReplies = Math.min(
  ...
  const posts = await Promise.all(
    odinPosts.map(async (post, index) => {
      ...
      return { ...createdPost, commentCount: post.commentCount };
    }),
  );

  // Create comments for each post
  for (const post of posts) {
    const totalCommentsNeeded = post.commentCount;
    const remainingComments = { count: totalCommentsNeeded };

    // Create top-level comments first, then nest some of them
    const topLevelComments = Math.ceil(totalCommentsNeeded * 0.6); // 60% top-level
    const actualTopLevel = Math.min(topLevelComments, totalCommentsNeeded);

    const createdTopLevelComments = [];

    for (let j = 0; j < actualTopLevel; j++) {
      const comment = await prisma.comment.create({
        data: {
      ...
      createdTopLevelComments.push(comment);
      remainingComments.count--;
    }

    // Now create nested replies for some top-level comments
    for (const topLevelComment of createdTopLevelComments) {
      if (remainingComments.count <= 0) break;

      // Randomly decide if this comment gets replies (70% chance)
      if (Math.random() < 0.7) {
        const maxNestingDepth = faker.number.int({ min: 2, max: 5 });
        await createNestedComments(
          post.id,
          topLevelComment.id,
          1,
  ...
  console.log('Successfully created posts and comments for r/theodinproject');
}
